...
export const DEFAULT_SUBTITLE_SETTING = {
  ...
  apiSlug: OPT_TRANS_DEEPSEEK, // 默认的字幕翻译接口使用 DeepSeek
  segSlug: OPT_TRANS_DEEPSEEK, // 智能 AI 断句/字幕合并使用 DeepSeek
  forceSubtitleRetranslate: false, // AI 断句服务与翻译服务不同时，是否强制使用翻译服务重翻译文
  chunkLength: 1000, // 新配置默认使用更短 AI 分块；已保存的用户值仍由存储配置优先覆盖
  longSentenceThreshold: 250, // 启用基于标点规则拆分的长句判定字符长度限制
...
export const DEFAULT_SETTING = {
  ...
  fabClickAction: 1, // 工具栏悬浮球按钮单击的默认响应行为 (1: 立即开启/关闭网页全文翻译, 0: 打开菜单)
