...
const DEFAULT_PARAMS = {
  maxDurationMs: 15000, // 单个句子的最大时长上限（毫秒），支持自然完整长句
  maxWords: 45, // 单个句子的最大单词数上限，避免过早截断
  sensitivity: 2.0, // 对停顿时间的评分灵敏度系数（Z-Score 的加权系数）
  punctuationBreakBonus: 3.5, // 强化真正句尾标点（.?!）断句的评分加分
  commaBreakBonus: 0.1, // 降低逗号加分，避免在逗号处将未说完的句子切断
  capitalBreakBonus: 0.5, // 下一单词首字母大写（可能代表新句子起始）的加分
  minBoundaryScore: 1.5, // 判定为断句边界的最小累加评分阈值，防止句子过碎
  minSentenceWords: 5, // 单个句子的最小单词数限制，防止句子过碎
  minSentenceDurationMs: 800, // 单个句子的最小时长限制（毫秒）
  forceBreakOnPunctuation: true, // 在标点符号处启用硬性条件强制断句
  forcePunctuationMinWords: 4, // 标点符号硬断句的最小单词数要求
  forcePunctuationMinDurationMs: 1000, // 标点符号硬断句的最小时长要求
};
