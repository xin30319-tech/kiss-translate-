...
describe("youtubeSubtitleProcessing", () => {
  ...
  test("preserves complete natural sentence with commas without premature splitting", () => {
    const flatEvents = Array.from({ length: 18 }, (_, index) => ({
      text: index === 0 ? "First," : index === 17 ? "end." : `word${index}`,
      start: index * 100,
      end: index * 100 + 80,
    }));
    ...
    expect(subtitles[0].text).toContain("end.");
  });
  ...
  test("reconstructs complete sentences across line-wrapped manual caption events", () => {
    const rawEvents = [
      {
        tStartMs: 26000,
        dDurationMs: 10720,
        segs: [
          {
            utf8: "And while everyone is distracted by handheld",
          },
        ],
      },
      {
        ...
        segs: [
          {
            utf8: "consoles, one has been building the only real escape route out.",
          },
        ],
      },
      {
      ...
      },
      {
        ...
        segs: [
          {
            utf8: "locally. Your data never leaves your machine.",
          },
        ],
      },
    ];
    ...
    expect(cues.map((c) => c.text)).toEqual([
      "And while everyone is distracted by handheld consoles, one has been building the only real escape route out.",
